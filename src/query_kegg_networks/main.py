...
def query_shortest_distances(input_folder_graphml, ko_metabolite_tsv, output_csv):
	...
	kos, metabolites = network_utils.read_ko_metabolite_map(ko_metabolite_tsv)

	all_network_data = {}
	for network_file in os.listdir(input_folder_graphml):
		network_file_path = os.path.join(input_folder_graphml, network_file)
		network_res = query_shortest_distance_single_network(network_file_path, ko_metabolite_tsv)
